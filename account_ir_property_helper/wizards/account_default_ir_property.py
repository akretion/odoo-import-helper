...
class AccountDefaultIrProperty(models.TransientModel):
    ...
    del_existing_property_res_partner = fields.Boolean(
        default=True, string="Delete Existing Specific Partner Account Properties"
    )
    del_existing_property_product_category = fields.Boolean(
        default=False,
        string="Delete Existing Specific Product Category Account Properties",
    )
    partner_receivable_account_id = fields.Many2one(
        "account.account",
        string="Partner Account Receivable",
        required=True,
        domain=(
    ...
    product_categ_income_account_id = fields.Many2one(
        "account.account",
        string="Product Category Income Account",
        required=True,
        domain=(
            "[('deprecated', '=', False), ('internal_type', '=', 'other'), "
            "('company_id', '=', company_id), ('is_off_balance', '=', False)]"
        ),
    )
    # target field: property_account_income_categ_id
    product_categ_expense_account_id = fields.Many2one(
        "account.account",
        string="Product Category Expense Account",
        required=True,
        domain=(
            "[('deprecated', '=', False), ('internal_type', '=', 'other'), "
            "('company_id', '=', company_id), ('is_off_balance', '=', False)]"
    ...
    partner_payable_account_property_id = fields.Many2one("ir.property", readonly=True)
    ...
    def run(self):
        ...
        for wizard_field, field_dict in WIZARD2FIELD.items():
            ...
            if self["del_existing_property_%s" % field_dict["model"].replace(".", "_")]:
                ...
                logger.info(
                    "Deleted %d ir.properties %s", len(properties), wizard_field
                )
