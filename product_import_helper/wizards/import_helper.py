...
class ImportHelper(models.TransientModel):
    ...
    def _prepare_speedy(self, aiengine="chatgpt"):
            ...
            for fc in self.env["account.product.fiscal.classification"].search([]):
                if len(fc.purchase_tax_ids) == 1 and len(fc.sale_tax_ids) == 1:
                    ...
                    if sale_rate != purchase_rate:
                        raise UserError(
                            _(
                                f"On fiscal classification {fc.display_name} "
                                f"(ID {fc.id}), the purchase tax rate ({purchase_rate})"
                                f" is different from the sale tax rate ({sale_rate})"
                            )
                        )
        ...
        for route_code, xmlid in route_code2xmlid.items():
            # I hope raise_if_not_found=False to avoid an error is the module is not
            # installed
        ...
        if inventory and stock_qty:
            if product.is_storable:
                ...
                speedy["logs"]["product.product"].append(
                    {
                        "msg": f"Cannot set stock_qty={stock_qty} on product with "
                        f"type={product.type}",
                        "value": stock_qty,
                        "vals": vals,
    ...
    # it can contain some special keys, which will be replaced by the corresponding real
    # key after processing:
    ...
    def _prepare_product_vals(self, vals, location_id, speedy):
        ...
        if vals.get("default_code"):
            if vals["default_code"] in speedy["product_default_code2name"]:
                speedy["logs"]["product.product"].append(
                    {
                        "msg": "PRODUCT NOT IMPORTED: internal reference "
                        f"'{vals['default_code']}' used on another product "
                        f"'{speedy['product_default_code2name'][vals['default_code']]}'",
        ...
        if vals.get("barcode"):
            ...
            if barcode in speedy["product_barcode2name"]:
                speedy["logs"]["product.product"].append(
                    {
                        "msg": "PRODUCT NOT IMPORTED: barcode "
                        f"'{barcode}' used on another product "
                        f"'{speedy['product_barcode2name'][barcode]}'",
                        "value": barcode,
                        "vals": vals,
            ...
            if len(barcode) in (8, 13, 14):
                if not is_valid(barcode):
                    speedy["logs"]["product.product"].append(
                        {
                            "msg": f"Barcode {barcode} has an invalid checksum",
                            "value": barcode,
                            "vals": vals,
                            "field": "product.product,barcode",
                        }
                ...
                speedy["logs"]["product.product"].append(
                    {
                        "msg": f"Barcode {barcode} has {len(barcode)} characters "
                        "(should be 8, 13 or 14 for an EAN barcode)",
                        "value": barcode,
                        "vals": vals,
                        "field": "product.product,barcode",
                    }
        ...
        if "vat_rate" in vals and "vat_rate2fc_id" in speedy:
            ...
            if not isinstance(vat_rate, int):
                speedy["logs"]["product.product"].append(
                    {
                        "msg": f"vat_rate key must be an integer, not {type(vat_rate)}",
                        "value": vat_rate,
                        "vals": vals,
                        "field": "product.product,barcode",
                        "reset": True,
            ...
            if vat_rate in speedy["vat_rate2fc_id"]:
                ...
                speedy["logs"]["product.product"].append(
                    {
                        "msg": f"{vat_rate} is not a know VAT rate "
                        f"({" ".join([
                                   str(x) for x in speedy['vat_rate2fc_id'
                                   ]])})",
                        "value": vat_rate,
                        "vals": vals,
                        "field": "product.product,barcode",
            ...
            if vals.get("supplier_currency"):
                if isinstance(vals["supplier_currency"], int):
                    ...
                    if currency in speedy["currency2id"]:
                        ...
                        speedy["logs"]["product.product"].append(
                            {
                                "msg": f"{currency} is not a known currency ISO code",
                                "value": currency,
                                "vals": vals,
        ...
        if vals.get("orderpoint_min_qty"):
            if not location_id:
                raise UserError(
                    _(
                        "location_id argument is not set and no warehouse in "
                        "company '%s'."
                    )
                    % self.env.company.display_name
                )
        ...
        for key in vals.keys():
            if key != "orderpoint_ids" and (
                key.startswith("supplier_") or key.startswith("orderpoint_")
            ):
                rvals.pop(key)
        return rvals
    ...
    def _match_and_update_account(self, acc_type, vals, speedy):
        ...
        for code, account_id in speedy["account_code2id"].items():
            if code.startswith(account_code):
                speedy["logs"]["product.product"].append(
                    {
                        "msg": f"Approximate match: account {account_code} has been "
                        f"matched with account {code}",
                        "value": account_code,
                        "vals": vals,
