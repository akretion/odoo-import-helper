...
class AccountChartGenerate(models.TransientModel):
    ...
    input_has_header_line = fields.Boolean(
        ...
        help="Enable this option if the first line of the XLSX file is a header line "
    ...
    def run(self):
        ...
        for row in sh.rows:
            ...
            if code and name:
                ...
                if code in custom_chart:
                    raise UserError(
                        _("Double entry in the chart of account: account '%s'.") % code
                    )
        ...
        self.write(
            {
                ...
                "out_csv_filename": f"account.account-{self.module}.csv",
            }
        )
        fout.close()
        ...
        action = {
            ...
            "url": f"web/content/?model=account.chart.generate&id={self.id}&filename_field=out_csv_filename&field=out_csv_file&download=true&filename={self.out_csv_filename}",  # noqa : E501
            "target": "new",
        }
        return action
