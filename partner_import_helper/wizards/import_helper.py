...
class ImportHelper(models.TransientModel):
            ...
            if fp_count:
                ...
                for fr_vat_type in speedy["fiscal_position"]["frvattype2id"].keys():
                    ...
                    if not fps:
                        raise UserError(
                            _(
                                "There are no fiscal position with fr_vat_type=%(fr_vat_type)s in company '%(company)s'.",
                                fr_vat_type=fr_vat_type,
    ...
    def _prepare_parent_child_partner_vals(
            ...
            if vals.get(phone_field):
                if speedy["o2m_phone"] and isinstance(vals[phone_field], list):
                    ...
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": "%s key should be a string, not %s"
                            % (phone_field, type(vals[phone_field]).__name__),
        ...
        if vals.get("email"):
            if speedy["o2m_phone"] and isinstance(vals["email"], list):
                ...
                speedy["logs"]["res.partner"].append(
                    {
                        "msg": "email key should be a string, not %s"
                        % type(vals["email"]).__name__,
                        "value": vals["email"],
                        "vals": vals,
        ...
        if country_id and country_id == speedy["fr_country_id"] and vals.get("zip"):
            ...
            if len(zipcode) != 5:
                speedy["logs"]["res.partner"].append(
                    {
                        "msg": "Zip code has %d chars. In France, they have 5 chars."
                        % len(zipcode),
                        "value": zipcode,
                        "vals": vals,
    ...
    def _prepare_partner_vals(
        ...
        if vals.get("vat") and (
            ...
            if vat:
                try:
                    ...
                    logger.warning(
                        "Could not perform VIES validation on VAT %s: %s", vat, e
                    )
                    speedy["logs"]["res.partner"].append(
                        {
            ...
            if not iban_is_valid(iban):
                ...
                if vals.get("bic"):
                    ...
                    if bic in speedy["bank"]["bic2id"]:
                        ...
                        speedy["logs"]["res.partner"].append(
                            {
                                "msg": "BIC not found in Odoo. New bank named '%s' created (ID %d)"
                                % (bank.name, bank.id),
                                "value": bic,
                                "vals": vals,
                                "field": "res.bank,bic",
                ...
                if vals.get("bank_ids"):
                    raise UserError(
                        _(
                            "vals contains both an 'iban' and a 'bank_ids' keys. This should never happen."
                        )
                    )
        ...
        if vals.get("siren") and hasattr(self.env["res.partner"], "siren"):
            ...
            if siren and vat:
                if vat[:2] != "FR":
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": "Partner has SIREN '%s', so it's VAT number should start with FR"
                            % siren,
                            "value": vat,
                            "vals": vals,
                            "field": "res.partner,vat",
                ...
                if vat[4:] != siren:
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": "Partner has SIREN '%s', so it must compose the 9 last digits of it's VAT number"
                            % siren,
                            "value": vat,
                            "vals": vals,
                            "field": "res.partner,vat",
        ...
        if vals.get("siret") and hasattr(self.env["res.partner"], "siret"):
            ...
            if siret and vat:
                if vat[:2] != "FR":
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": "Partner has SIRET '%s', so it's VAT number should start with FR"
                            % siret,
                            "value": vat,
                            "vals": vals,
                            "field": "res.partner,vat",
                ...
                if vat[4:] != siret[:9]:
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": "Partner has SIRET '%s', so the 9 first digits of the SIRET must compose the 9 last digits of it's VAT number"
                            % siret,
                            "value": vat,
                            "vals": vals,
                            "field": "res.partner,vat",
        ...
        if vals.get("siren") and vals.get("siret"):
            if not vals["siret"].startswith(vals["siren"]):
                speedy["logs"]["res.partner"].append(
                    {
                        "msg": "Partner has both a SIREN and a SIRET, so its SIRET should start with its SIREN (%s)"
                        % vals["siren"],
                        "value": vals["siret"],
                        "vals": vals,
                        "field": "res.partner,siret",
        ...
        if country_id:
            ...
            if iban and not iban.startswith(country_code):
                speedy["logs"]["res.partner"].append(
                    {
                        "msg": "The country prefix of the IBAN doesn't match the country code '%s'"
                        % country_code,
                        "value": iban,
                        "vals": vals,
                        "field": "res.partner.bank,acc_number",
            ...
            if vals.get("customer_payment_term_code"):
                if vals["customer_payment_term_code"] in speedy["payment_term"]:
                    ...
                    if customer_payment_term_int in speedy["payment_term"]:
                        ...
                        speedy["logs"]["res.partner"].append(
                            {
                                "msg": "Payment term code '%s' doesn't exist"
                                % vals["customer_payment_term_code"],
    ...
    def _phone_number_clean(self, number, country_code, phone_field, vals, speedy):
        try:
            ...
            speedy["logs"]["res.partner"].append(
                {
                    "msg": "Failed to reformat with country '%s': %s"
                    % (country_code, e),
                    "value": number,
                    "vals": vals,
                    "field": "res.partner,%s" % phone_field,
