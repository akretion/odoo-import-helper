...
class ImportHelper(models.TransientModel):
    ...
    def _prepare_speedy(self, aiengine="chatgpt"):
        ...
        speedy.update(
            {
                ...
                "industry_name2id": {},
                "fiscal_position": {},
                # _phone_get_number_fields() is a method of phone_validation that
        ...
        for bank in (
            ...
            .search_read([("bic", "!=", False)], ["name", "bic"])
        ):
            bic = bank["bic"].upper()
            speedy["bank"]["bic2id"][bic] = bank["id"]
            speedy["bank"]["bic2name"][bic] = bank["name"]
        for indus in (
            self.env["res.partner.industry"]
            .with_context(active_test=False)
            .search_read([("name", "!=", False)], ["name"])
        ):
            speedy["industry_name2id"][indus["name"]] = indus["id"]
        if (
            self.env.company.country_id.code == "FR"
            and hasattr(self.env["res.partner"], "property_account_position_id")
            and hasattr(self.env["account.fiscal.position"], "fr_vat_type")
        ):
            fp_count = self.env["account.fiscal.position"].search_count(
            ...
            if fp_count:
                ...
                for fr_vat_type in speedy["fiscal_position"]["frvattype2id"].keys():
                    fps = self.env["account.fiscal.position"].search_read(
                        [
                            ("company_id", "=", self.env.company.id),
                            ("fr_vat_type", "=", fr_vat_type),
                        ],
                        ["name"],
                    )
                    if not fps:
                        raise UserError(
                            _(
                                "There is no fiscal position with "
                                "fr_vat_type=%(fr_vat_type)s in company '%(company)s'.",
                                fr_vat_type=fr_vat_type,
    ...
    def _create_partner(
        self, vals, speedy, email_check_deliverability=True, create_bank=True
    ):
        ...
        vals["id"] = partner.id
        logger.info(
            "New partner created: %s ID %d from line %d",
    ...
    @api.model
    def _prepare_parent_child_partner_vals(
        self,
        vals,
        parent_or_child,
        speedy,
        email_check_deliverability=True,
        parent_country_id=False,
    ):
        assert vals
        ...
        if (
            vals.get("country_name")
            and isinstance(vals["country_name"], str)
            and not vals.get("country_id")
        ):
        ...
        if (
            ...
            and vals.get("title_code")
            and isinstance(vals["title_code"], str)
            and not vals.get("title")
        ):
            ...
            if vals.get(phone_field):
                if speedy["o2m_phone"] and isinstance(vals[phone_field], list):
                    ...
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": f"{phone_field} key should be a string, "
                            f"not {type(vals[phone_field]).__name__}",
                            "value": vals[phone_field],
                            "vals": vals,
                            "field": f"res.partner,{phone_field}",
                            "reset": True,
                        }
                    )
            ...
            if speedy["o2m_phone"] and isinstance(vals["email"], list):
                ...
                for email in vals["email"]:
                    if email and isinstance(email, str):
                        email = self._email_validate(
                            email, email_check_deliverability, vals, speedy
                        )
                        if email:
                            vals["phone_ids"].append(
                                Command.create(
                ...
                vals["email"] = self._email_validate(
                    vals["email"], email_check_deliverability, vals, speedy
                )
            else:
                speedy["logs"]["res.partner"].append(
                    {
                        "msg": "email key should be a string, "
                        f"not {type(vals['email']).__name__}",
                        "value": vals["email"],
                        "vals": vals,
        ...
        if country_id and country_id == speedy["fr_country_id"] and vals.get("zip"):
            ...
            if len(zipcode) != 5:
                speedy["logs"]["res.partner"].append(
                    {
                        "msg": f"Zip code has {len(zipcode)} chars. In France, they "
                        "have 5 chars.",
                        "value": zipcode,
                        "vals": vals,
    ...
    # it can contain some special keys, which will be replaced by the corresponding
    # real key after processing:
    ...
    def _prepare_partner_vals(
        ...
        # is_company
        if not vals.get("is_company") and (
            vals.get("vat") or vals.get("siren") or vals.get("siret")
        ):
            if vals.get("vat"):
        ...
        vat = False
        if vals.get("vat") and (
            not country_id or country_id in speedy["eu_country_ids"]
        ):
            vat = vals["vat"].upper()
            # clean VAT
            ...
            if vat:
                try:
                    ...
                    logger.warning(
                        f"Could not perform VIES validation on VAT {vat}: {e}"
                    )
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": f"Could not perform VIES validation: {e}",
                            "value": vat,
                            "vals": vals,
                            "field": "res.partner,vat",
                        }
            ...
            if not iban_is_valid(iban):
                ...
                if vals.get("bic"):
                    ...
                    if len(bic) not in (8, 11):
                        speedy["logs"]["res.partner"].append(
                            {
                                "msg": "Wrong BIC: length is %d, should be 8 or 11"
                                % len(bic),
                                "value": bic,
                                "vals": vals,
                                "field": "res.bank,bic",
                    ...
                    if bic in speedy["bank"]["bic2id"]:
                        ...
                        speedy["logs"]["res.partner"].append(
                            {
                                "msg": "BIC not found in Odoo. New bank named "
                                f"'{bank.name}' created (ID {bank.id})",
                                "value": bic,
                                "vals": vals,
                                "field": "res.bank,bic",
                            }
                ...
                if vals.get("bank_ids"):
                    raise UserError(
                        _(
                            "vals contains both an 'iban' and a 'bank_ids' keys. "
        ...
        if vals.get("siren_or_siret") and hasattr(self.env["res.partner"], "siret"):
            ...
            if siren_or_siret:
                if len(siren_or_siret) == 14:
                    ...
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": "SIREN/SIRET has a length of %d instead of 9 or 14"
                            % len(siren_or_siret),
        ...
        if vals.get("siren") and hasattr(self.env["res.partner"], "siren"):
            ...
            if siren and vat:
                if vat[:2] != "FR":
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": f"Partner has SIREN '{siren}', so it's VAT number "
                            "should start with FR",
                            "value": vat,
                            "vals": vals,
                            "field": "res.partner,vat",
                        }
                ...
                if vat[4:] != siren:
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": f"Partner has SIREN '{siren}', so it must compose "
                            "the 9 last digits of it's VAT number",
                            "value": vat,
                            "vals": vals,
                            "field": "res.partner,vat",
                        }
        ...
        if vals.get("siret") and hasattr(self.env["res.partner"], "siret"):
            ...
            if siret and vat:
                if vat[:2] != "FR":
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": f"Partner has SIRET '{siret}', so it's VAT number "
                            "should start with FR",
                            "value": vat,
                            "vals": vals,
                            "field": "res.partner,vat",
                ...
                if vat[4:] != siret[:9]:
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": f"Partner has SIRET '{siret}', so the 9 first "
                            "digits of the SIRET must compose the 9 last digits of "
                            "it's VAT number",
                            "value": vat,
                            "vals": vals,
                            "field": "res.partner,vat",
        ...
        if vals.get("siren") and vals.get("siret"):
            if not vals["siret"].startswith(vals["siren"]):
                speedy["logs"]["res.partner"].append(
                    {
                        "msg": "Partner has both a SIREN and a SIRET, so its SIRET "
                        f"should start with its SIREN ({vals['siren']})",
                        "value": vals["siret"],
                        "vals": vals,
                        "field": "res.partner,siret",
                        "reset": True,
        ...
        if vals.get("industry_name"):
            if vals["industry_name"] not in speedy["industry_name2id"]:
                indus = self.env["res.partner.industry"].create(
                    self._prepare_industry(vals, speedy)
                )
        ...
        if country_id:
            ...
            if vat and country_id in speedy["eu_country_ids"]:
                ...
                if expected_country_code != country_code:
                    speedy["logs"]["res.partner"].append(
                        {
                            "msg": f"Given the VAT number ({vat}), the country code "
                            f"should be '{expected_country_code}' but it is "
                            f"'{country_code}'",
                            "value": vat,
                            "vals": vals,
                            "field": "res.partner,vat",
            ...
            if iban and not iban.startswith(country_code):
                speedy["logs"]["res.partner"].append(
                    {
                        "msg": "The country prefix of the IBAN doesn't match the "
        ...
        if speedy.get("payment_term"):
            if vals.get("customer_payment_term_code"):
                if vals["customer_payment_term_code"] in speedy["payment_term"]:
                    vals["property_payment_term_id"] = speedy["payment_term"][
                        vals["customer_payment_term_code"]
                    ]
                else:
                    # try to convert to int
                    try:
                        customer_payment_term_int = int(
                            vals["customer_payment_term_code"]
                        )
                    except ValueError:
                    ...
                    if customer_payment_term_int in speedy["payment_term"]:
                        vals["property_payment_term_id"] = speedy["payment_term"][
                            customer_payment_term_int
                        ]
                    else:
                        speedy["logs"]["res.partner"].append(
                            {
                                "msg": "Payment term code "
                                f"'{vals['customer_payment_term_code']}' "
                                "doesn't exist",
            ...
            if vals.get("supplier_payment_term_code"):
                if vals["supplier_payment_term_code"] in speedy["payment_term"]:
                    vals["property_supplier_payment_term_id"] = speedy["payment_term"][
                        vals["supplier_payment_term_code"]
                    ]
                else:
                    # try to convert to int
                    try:
                        supplier_payment_term_int = int(
                            vals["supplier_payment_term_code"]
                        )
                    except ValueError:
                        supplier_payment_term_int = None
                    if supplier_payment_term_int in speedy["payment_term"]:
                        vals["property_supplier_payment_term_id"] = speedy[
                            "payment_term"
                        ][supplier_payment_term_int]
                    else:
                        speedy["logs"]["res.partner"].append(
                            {
                                "msg": "Payment term code "
                                f"'{vals['supplier_payment_term_code']}' "
                                "doesn't exist",
        ...
        if (
            hasattr(self.env["res.partner"], "property_account_position_id")
            and speedy["fiscal_position"].get("frvattype2id")
            and country_id
        ):
            if country_id in (speedy["fr_country_id"], speedy["mc_country_id"]):
                vals["property_account_position_id"] = speedy["fiscal_position"][
                    "frvattype2id"
                ]["france"]
                # DOMs
                if (
                ...
                if vals.get("is_company"):
                    ...
                    if not vals.get("vat"):
                        speedy["logs"]["res.partner"].append(
                            {
                                "msg": "The fiscal position Intra-EU B2B has been set "
                                "but the partner has no VAT.",
                                "value": speedy["fiscal_position"]["id2name"][
                                    vals["property_account_position_id"]
                                ],
                                "vals": vals,
        ...
        # rvals will be used for create(), so we need to remove all the keys are don't
        # exist on res.partner
    ...
    def _phone_number_clean(self, number, country_code, phone_field, vals, speedy):
        try:
            ...
            speedy["logs"]["res.partner"].append(
                {
                    "msg": f"Failed to reformat with country '{country_code}': {e}",
                    "value": number,
                    "vals": vals,
                    "field": f"res.partner,{phone_field}",
                }
