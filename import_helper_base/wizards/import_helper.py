...
class ImportHelper(models.TransientModel):
    ...
    def _prepare_speedy(self, aiengine="chatgpt"):
        ...
        if aiengine == "chatgpt":
            ...
            if not openai_api_key:
                raise UserError(
                    _(
                        "Missing entry openai_api_key in the Odoo server configuration "
                        "file."
                    )
                )
            ...
            if len(answer) == 2:
                ...
                if country_code in cyd["code2id"]:
                    ...
                    speedy["logs"][model].append(
                        dict(
                            log,
                            msg="Country name could not be found in Odoo. ChatGPT said "
                            f"ISO code was '{country_code}', which matched to "
                            f"'{cyd["code2name"][country_code]}'",
                        )
                    )
                    country_id = cyd["code2id"][country_code]
                    ...
                    speedy["logs"][model].append(
                        dict(
                            log,
                            msg="Country name could not be found in Odoo. ChatGPT said "
                            "ISO code was '{country_code', which didn't match to "
                            "any country",
                        ),
                        reset=True,
                    )
                ...
                speedy["logs"][model].append(
                    dict(
                        log,
                        msg="ChatGPT didn't answer a 2 letter country code but "
                        f"'{answer}'",
                        reset=True,
                    )
                )
    ...
    def _convert_logs2html(self, speedy):
        html = (
            '<p><small>For the logs in <span style="color: red">red</span>, the data '
            "was <b>not imported</b> in Odoo</small><br/>"
        )
        ...
        for obj_name, log_list in speedy["logs"].items():
            ...
            for line, logs in line2logs.items():
                ...
                for log in logs:
                    log_labels.append(
                        f'<li style="color: {log.get("reset") and "red" or "black"}">'
                        f'<b>{self._field_label(log["field"], speedy)}</b>: '
                        f'<b>{log["value"]}</b> - {log["msg"]}</li>'
                    )
                h3 = f"Line {line}"
                if log["vals"].get("id"):
